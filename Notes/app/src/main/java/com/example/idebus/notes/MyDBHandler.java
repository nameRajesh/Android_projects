...
public class MyDBHandler extends SQLiteOpenHelper {
    ...
    public String[] databaseToString(){
        ...
        while (!recordSet.isAfterLast()) {
            ...
            if (recordSet.getString(recordSet.getColumnIndex("productname")) != null) {
                list.add(recordSet.getString(recordSet.getColumnIndex("productname")));
            }
            recordSet.moveToNext();
            c++;
        }
        db.close();
        String[] dbString = list.toArray(new String[0]);
        return dbString;
    }
    ...
    public boolean checkString(String text){
        ...
        while (!recordSet.isAfterLast()) {
            // null could happen if we used our empty constructor
            String s = recordSet.getString(recordSet.getColumnIndex("productname"));
            if (s.equals(text)) {
                count++;
            }
            recordSet.moveToNext();
